...
function carteiraLogos($ticker, $atual, array $historico) {
    ...
    if (preg_match('/^[A-Z0-9.-]{1,20}$/D', $ticker)) {
        foreach (['png', 'webp', 'jpg', 'svg'] as $ext) {
            $relativo = '../assets/img/acoes/internacionais/' . $ticker . '.' . $ext;
...
function internacionalCotacao($ticker) {
    ...
    $q = mercadoApi()->stocks([$ticker], 'USD')[$ticker];
    return [$q['price'], $q['logo']];
}
...
if ($estrutura_ok) {
    try {
        ...
        $av_cotacoes = mercadoApi()->stocks(array_column($posicoes, 'ticker'), 'USD');
...
$cotacao_usd = $cambio['USD']['price'];
$cotacao_eur = $cambio['EUR']['price'];
?>
...
foreach ($posicoes as $av_posicao) {
    $av_custo = bcadd($av_custo, $av_posicao['total_investido_usd'], 24);
}
$av_ok = $estrutura_ok && $erro_carteira === '';
$av_mercado = '0';
foreach ($posicoes as $av_posicao) {
    if ($av_posicao['valor_mercado_atual'] !== null) {
        $av_mercado = bcadd($av_mercado, bcmul($av_posicao['valor_mercado_atual'], $av_posicao['quantidade_total'], 24), 24);
    }
}
$av_valores_ok = $av_ok && $cotacoes_completas;
$av_ganho = $av_valores_ok ? bcsub($av_mercado, $av_custo, 24) : null;
...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/style-acoes-internacionais.css?v=20260913">
</head>
<body class="av-body" data-modal-inicial="<?= internacionalEscape($av_modal) ?>">
    <?php include('../includes/header.php'); ?>
    <?php include('../includes/menu.php'); ?>
    <main class="av-page">
        <a class="av-back" href="acoes.php">← Resumo de Ações</a>
        <div class="av-top">
            <div><h1>Ações Internacionais</h1><p>Gerencie suas posições em dólares e operações fracionadas.</p></div>
        ...
        <?php if ($mensagem !== '') { ?><p class="av-success" role="status"><?= internacionalEscape($mensagem) ?></p><?php } ?>
        <?php if ($erro !== '') { ?><p class="av-alert" role="alert"><?= internacionalEscape($erro) ?></p><?php } ?>
        <?php if ($erro_carteira !== '') { ?><p class="av-alert" role="alert"><?= internacionalEscape($erro_carteira) ?></p><?php } ?>
        <div class="av-cambio" aria-label="Cotação do dólar e euro">
            <strong>Câmbio de referência</strong>
            <span>USD/BRL <b><?= $cotacao_usd !== null ? 'R$ ' . number_format($cotacao_usd, 4, ',', '.') : 'Indisponível' ?></b></span>
            <span>EUR/BRL <b><?= $cotacao_eur !== null ? 'R$ ' . number_format($cotacao_eur, 4, ',', '.') : 'Indisponível' ?></b></span>
            <small>Carteira apresentada em USD.</small><details class="av-quote-info"><summary>Fonte e horário do câmbio<?= $cambio['USD']['stale'] || $cambio['EUR']['stale'] ? ' · atualização pendente' : '' ?></summary><p><?= internacionalEscape(mercadoLegenda($cambio['USD'])) ?></p><p>EUR: <?= internacionalEscape(mercadoLegenda($cambio['EUR'])) ?></p></details>
        </div>
        <section class="av-card" aria-labelledby="carteira-titulo">
            <div class="av-summary">
                <div><h2 id="carteira-titulo">Posição consolidada</h2><small><?= $av_ok ? count($posicoes) . ' ativo(s) em carteira' : 'Consulta indisponível' ?> · USD</small></div>
                <div class="av-metrics"><div class="av-summary-value"><span>Custo das posições atuais</span><strong><?= $av_ok ? 'US$ ' . number_format($av_custo, 2, ',', '.') : 'Indisponível' ?></strong></div>
<div class="av-summary-value"><span>Valor atual da carteira</span><strong><?= $av_valores_ok ? 'US$ ' . number_format($av_mercado, 2, ',', '.') : 'Indisponível' ?></strong><small>Pelas cotações consultadas</small></div>
<div class="av-summary-value"><span>Resultado não realizado</span><strong class="<?= $av_ganho !== null && (float)$av_ganho < 0 ? 'av-negative' : 'av-positive' ?>"><?= $av_ganho !== null ? ((float)$av_ganho < 0 ? '- ' : '+ ') . 'US$ ' . number_format(abs((float)$av_ganho), 2, ',', '.') : 'Indisponível' ?></strong><small><?= carteiraPercentual($av_ganho, $av_custo) ?></small></div>
</div>
            </div>
            ...
            <div class="av-scroll" tabindex="0" role="region" aria-label="Tabela de posições">
                <table class="tabela-acoes">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <?php $av_logos = carteiraLogos($row['ticker'], $row['logo'] ?? null, $ops ?? []); ?>
                                    <span class="av-logo" data-logos="<?= internacionalEscape(json_encode($av_logos, JSON_UNESCAPED_SLASHES | JSON_INVALID_UTF8_SUBSTITUTE)) ?>" title="<?= internacionalEscape($row['ticker']) ?>">
                                        <span class="av-logo-fallback" aria-hidden="true"><?= internacionalEscape(substr($row['ticker'], 0, 4)) ?></span>
                                        <img class="av-logo-img" alt="Logo de <?= internacionalEscape($row['ticker']) ?>" width="30" height="30" hidden referrerpolicy="no-referrer">
                                    </span>
                                </td>
                                ...
                                <td><?= $row['valor_mercado_atual'] !== null ? 'US$ ' . number_format($row['valor_mercado_atual'], 2, ',', '.') : 'Indisponível' ?><details class="av-quote-info"><summary><?= $row['api']['stale'] ? 'Preço salvo · atualização pendente' : 'Fonte e horário' ?></summary><?= internacionalEscape(mercadoLegenda($row['api'])) ?></details></td>
                                <td><?php if ($row['resultado'] === null) { ?>Indisponível<?php } elseif (bccomp($row['resultado'], '0', 24) >= 0) { ?><span class="av-positive">+ US$ <?= number_format($row['resultado'], 2, ',', '.') ?></span><?php } else { ?><span class="av-negative">- US$ <?= number_format(abs($row['resultado']), 2, ',', '.') ?></span><?php } ?><small class="av-return-percent"><?= carteiraPercentual($row['resultado'], $row['total_investido_usd']) ?></small></td>
                            </tr>
                            <?php } ?>
                            <tr>
                                <td colspan="6" style="text-align:right;"><strong>Total:</strong></td>
                                <td><?php if (!$cotacoes_completas) { ?>Indisponível<?php } elseif (bccomp((string) $total_resultado, '0', 24) >= 0) { ?><span class="av-positive">+ US$ <?= number_format($total_resultado, 2, ',', '.') ?></span><?php } else { ?><span class="av-negative">- US$ <?= number_format(abs($total_resultado), 2, ',', '.') ?></span><?php } ?><small class="av-return-percent"><?= carteiraPercentual($cotacoes_completas ? $total_resultado : null, $av_custo) ?></small></td>
                            </tr>
                        </tbody>
                    </table>
            </div>
...
<p class="av-note">O resultado corresponde às posições atuais. O lucro ou prejuízo das vendas não está incluído.</p>
    ...
    <dialog id="modal-operacao" class="av-dialog" aria-labelledby="operacao-titulo">
        <div class="av-dialog-top"><div><h2 id="operacao-titulo">Registrar operação</h2><p>Ações Internacionais · USD</p></div><button type="button" data-close aria-label="Fechar operação">×</button></div>
        <div class="av-dialog-content">
            <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && $erro !== '') { ?><p class="av-alert" role="alert"><?= internacionalEscape($erro) ?></p><?php } ?>
            <form class="form-acoes" method="POST" id="form-internacional">
                        ...
                        <input type="text" id="op-ticker" name="ticker" maxlength="20" placeholder="Ticker (ex: AAPL, MSFT)" value="<?= internacionalEscape($form['ticker']) ?>" required>
                        ...
                        <input type="date" id="op-data" name="data" value="<?= internacionalEscape($form['data']) ?>" required>
                        <div class="acao-buttons">
                            <button type="button" aria-pressed="<?= $form['acao'] === 'compra' ? 'true' : 'false' ?>" class="btn-acao compra<?= $form['acao'] === 'compra' ? ' active' : '' ?>" onclick="selecionarAcao('compra')">Compra</button>
                            <button type="button" aria-pressed="<?= $form['acao'] === 'venda' ? 'true' : 'false' ?>" class="btn-acao venda<?= $form['acao'] === 'venda' ? ' active' : '' ?>" onclick="selecionarAcao('venda')">Venda</button>
                        </div>
                        <input type="hidden" name="acao" id="acao" value="<?= internacionalEscape($form['acao']) ?>">
                        <div class="av-form-actions"><button type="button" data-close>Cancelar</button><button class="av-primary" type="submit" <?= !$estrutura_ok ? 'disabled' : '' ?>>Registrar</button>
                    </div>
                    </form>
        </div>
    </dialog>
    <dialog id="modal-historico" class="av-dialog av-dialog-wide" aria-labelledby="historico-titulo">
        ...
        <div class="av-dialog-content">
            <form method="GET" class="av-search">
                ...
                <div><input type="text" name="busca" id="busca" value="<?= internacionalEscape($busca) ?>" placeholder="AAPL ou 09/09/2026"><button type="submit" class="av-primary">Pesquisar</button><a class="av-button" href="acoes_internacionais.php">Limpar</a></div>
            </form>
